...
// ======================================================
// ✅ Єдина правильна функція для реєстрації всіх веб-хендлерів
void registerWebHandlers(AsyncWebServer &server) {

  // 🏠 Головна сторінка
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    handleRootPage(request);   // викликає функцію, що формує головну сторінку
  });

  // 📋 Сторінка з переліком лог-файлів
  server.on("/logs", HTTP_GET, [](AsyncWebServerRequest *request) {
    handleLogList(request);    // викликає функцію, що формує HTML-сп
